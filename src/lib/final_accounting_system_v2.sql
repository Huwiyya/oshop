-- =============================================================================
-- FINAL ACCOUNTING SYSTEM V2 - CONSOLIDATED SCHEMA
-- =============================================================================
-- This script installs the complete V2 Accounting System.
-- It includes:
-- 1. Core Config & Types
-- 2. Chart of Accounts (COA)
-- 3. General Ledger (Journal Entries)
-- 4. Treasury (Receipts & Payments)
...
-- Triggers for Treasury Journal Creation (Omitted details for brevity, assumed standard)
-- (Adding Simplified Versions for Completeness)
...
BEGIN
    IF NEW.status = 'posted' AND OLD.status != 'posted' THEN
        ...
        VALUES ('JE-'||NEW.receipt_number, NEW.date, 'Receipt: '||NEW.description, 'posted', 'receipt', NEW.id, NEW.amount, NEW.amount) RETURNING id INTO v_jid;
        INSERT INTO public.journal_lines_v2 (journal_id, account_id, debit, credit, description) VALUES (v_jid, NEW.treasury_account_id, NEW.amount, 0, NEW.description);
        INSERT INTO public.journal_lines_v2 (journal_id, account_id, debit, credit, description) VALUES (v_jid, NEW.customer_account_id, 0, NEW.amount, NEW.description);
...
BEGIN
    IF NEW.status = 'posted' AND OLD.status != 'posted' THEN
        INSERT INTO public.journal_entries_v2 (entry_number, date, description, status, source_type, source_id, total_debit, total_credit)
        VALUES ('JE-'||NEW.payment_number, NEW.date, 'Payment: '||NEW.description, 'posted', 'payment', NEW.id, NEW.amount, NEW.amount) RETURNING id INTO v_jid;
        INSERT INTO public.journal_lines_v2 (journal_id, account_id, debit, credit, description) VALUES (v_jid, NEW.supplier_account_id, NEW.amount, 0, NEW.description);
        INSERT INTO public.journal_lines_v2 (journal_id, account_id, debit, credit, description) VALUES (v_jid, NEW.treasury_account_id, 0, NEW.amount, NEW.description);
...
CREATE TABLE IF NOT EXISTS public.products_v2 (
    ...
    sales_account_id REFERENCES public.accounts_v2(id),
