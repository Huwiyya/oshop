-- ============================================
-- FIX: Update Sales and Purchase RPCs to use 'inventory_items' instead of 'products'
-- The error "relation 'products' does not exist" occurs because the table is named 'inventory_items' (or 'products_v4').
-- We will strictly use 'inventory_items' which is verified to be used in other parts of the system.
-- FIX 2: Corrected loop variables to be RECORD types to avoid ERROR 42601.
-- FIX 3: Removed 'is_hidden' (boolean) parameter from create_journal_entry_rpc calls to match current schema.
-- ============================================
...
BEGIN
    ...
    LOOP
        ...
        INTO 
            ...
            v_cogs_account_id
        FROM inventory_items -- FIXED: Use inventory_items
        ...
        IF v_revenue_account_id IS NULL THEN
             SELECT id INTO v_revenue_account_id FROM accounts WHERE account_code = '410001'; -- Default Sales
        END IF;
        ...
        ELSE
            ...
            IF v_inventory_account_id IS NULL THEN
                SELECT id INTO v_inventory_account_id FROM accounts WHERE account_code = '113001'; -- Default Inventory
            END IF;
            IF v_cogs_account_id IS NULL THEN
                SELECT id INTO v_cogs_account_id FROM accounts WHERE account_code = '510001'; -- Default COGS
            END IF;
            ...
            IF (item_rec->'selectedLayerIds') IS NOT NULL AND jsonb_array_length(item_rec->'selectedLayerIds') > 0 THEN
                FOR loop_rec IN SELECT val FROM jsonb_array_elements_text(item_rec->'selectedLayerIds') AS val
                LOOP
                     layer_id_text := loop_rec.val;
    ...
    LOOP
        ...
        IF v_item_type = 'service' THEN
            ...
            IF v_expense_account_id IS NULL THEN
                 SELECT id INTO v_target_account_id FROM accounts WHERE account_code = '510001';
            ELSE
        ...
        ELSE
            ...
            IF v_inventory_account_id IS NULL THEN
                SELECT id INTO v_target_account_id FROM accounts WHERE account_code = '113001';
            ELSE
            ...
            IF jsonb_array_length(item_rec->'cardNumbers') > 0 THEN
                FOR loop_rec IN SELECT val FROM jsonb_array_elements_text(item_rec->'cardNumbers') AS val
                LOOP
                    card_num := loop_rec.val;
                    WITH new_layer AS (
