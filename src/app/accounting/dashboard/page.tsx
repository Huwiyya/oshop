        ...
        <Card className={`border shadow-sm hover:shadow-md transition-shadow ${href ? 'cursor-pointer' : ''}`}>
            <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                    ...
                    {trend && (
                        <div className={`flex items-center text-sm font-medium ${trend === 'positive' ? 'text-emerald-600' : 'text-red-500'
                            }`}>
                            {trend === 'positive' ? <ArrowUpRight className="w-4 h-4 ml-1" /> : <ArrowDownRight className="w-4 h-4 ml-1" />}
                            {trend === 'positive' ? 'إيجابي' : 'سلبي'}
                        </div>
                    )}
                </div>
                <div>
